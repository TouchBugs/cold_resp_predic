...
Thetarget = f'长{freeze_GRU}排序128-{threathhold}'
print(Thetarget)

best_acc = 0.5
root_dir = '/Data4/gly_wkdir/coldgenepredict/raw_sec/S_italica/CNN/长短分开训练/'
loss_png = root_dir + f'长loss-lr{lr}-wd{weight_decay}-ep{epochs}-{TheTime}-{Thetarget}.png'
acc_png = root_dir + f'长acc-lr{lr}-wd{weight_decay}-ep{epochs}-{TheTime}-{Thetarget}.png'
precision_png = root_dir + f'长precision-lr{lr}-wd{weight_decay}-ep{epochs}-{TheTime}-{Thetarget}.png'
...
for epoch in range(epochs):
    ...
    if val_accs[-1] > best_acc:
        best_acc = val_accs[-1]
        torch.save(model.state_dict(), root_dir + f'Zm长-model({epoch}_{epochs}-{best_acc:.4f}-{val_precision:.4f}-{val_recall:.4f}-{val_f1:.4f}-{lr}-{weight_decay}-{TheTime}--{Thetarget}).pth')
